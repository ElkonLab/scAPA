...
path.to.config = "/home/ubuntu/scAPA/scAPA.shell.script"
...
write(FilterBAM.command, paste0(path.to.files, "/scAPA/", "step1.sh"))
...
print(index.command)
write(index.command, paste0(path.to.files, "/scAPA/", "step2.sh"))
...
write(umi_tools.command, paste0(path.to.files, "/scAPA/", "step3.sh"))
...
write("rm temp/UB.*.bam*",  paste0(path.to.files, "/scAPA/", "step3-cleanup.sh"))
...
write(makeTagDirectory.command, paste0(path.to.files, "/scAPA/", "step4.sh"))
...
write(findPeaks.command, paste0(path.to.files, "/scAPA/", "step5.sh"))
...
# Peak analysis -----------------------------------------------------------
write(paste("Rscript /home/ruser/peak_analysis.R", path.to.files, bedtools.path, org, sep=" "), 
      "step6.sh")
#setwd("./temp")
...
samtools.merge.command <- paste0(samtools.path, "samtools merge merged.bam ",
                                dedup.bams,
                                " &> ../Log.files/samtoolsmerge.out")

write(samtools.merge.command, paste0(path.to.files, "/scAPA/", "step7.sh"))
...
write(wig.plus.command, paste0(path.to.files, "/scAPA/", "step8.sh"))

wig.minus.command <- paste0(bedtools.path, "bedtools genomecov -ibam merged.bam ",
                            "-bg -strand - | awk 'BEGIN {OFS = \"\t\"}{print $1",
                            ", $2, $3, $4, \".\", \"-\"}' >> merged.wig")
write(wig.plus.command, paste0(path.to.files, "/scAPA/", "step9.sh"))

#system(command = wig.minus.command, wait = T)
#system("rm merged.bam", wait = T)

write("rm merged.bam", paste0(path.to.files, "/scAPA/", "step9-cleanup.sh"))
...
write(intersect.wig.command, paste0(path.to.files, "/scAPA/", "step10.sh"))

write("rm merged.wig", paste0(path.to.files, "/scAPA/", "step10-cleanup.sh"))

write(paste("Rscript /home/ruser/scAPA_mclust.R", path.to.files, c, sep= " "), "step11.sh")
...
if (wig | !sc | ChangePoint) {
    ...
    for (j in 1:length(samples.vector)) {
        ...
        for (i in 1:length(list.cells)) {
            ...
            FilterBAMbyTag.command <- paste0(drop.seq.tools.path,
                                             ...
                                             "FilterBamByTag_",
                tagValuefile)
            write(FilterBAMbyTag.command, paste0(path.to.files, "/scAPA/", "step13_", 
                                                 tagValuefile, ".sh"))
    ...
    for (i in 1:length(clusters)) {
        ...
        write(samtools.merge.command, paste0(path.to.files, "/scAPA/", "step14_", 
                                             clusters[i], ".sh"))
        #write_log_start(f = "../scAPA.script.log",
...
if (sc) {
    ...
    for (j in 1:length(samples.vector)) {
        ...
        split_bams <- function(x) {
            ...
            #system(command = FilterBAMbyTag.command, wait = T)
            write(FilterBAMbyTag.command, paste0(path.to.files, "/scAPA/", "step12.sh"))
        }
        #if(c > 1){
