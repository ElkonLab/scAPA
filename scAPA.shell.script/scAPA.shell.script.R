...
if (((script.args[1] == "-h") | (script.args[1] == "--help") | (is.na(script.args[1])))) {
  cat("\n Usage: scAPA.script.R -p <path.to.files> -org <organism> [options]\n\n",
      ...
      "\n\nFiltering Options introns\n\n\t-Icpm\tConsider only peaks with more than",
...
if(wig | !sc){
  ...
  for (i in 1:length(list.cells)){
    ...
    FilterBAMbyTag.command <- paste0(drop.seq.tools.path,"FilterBamByTag ",
                                     "TAG=CB I=dedup.", sample, ".bam O=",
                                     sample, "_", cluster,".bam ",
                                     "TAG_VALUES_FILE=", tagValuefile,
                                     " &> ../Log.files/FilterBamByTag_",
                                     tagValuefile)
    ...
    write_log(f = "../scAPA.script.log",
              stage = paste0("FilterBAMbyTag.command: ", sample, "_",
                             cluster,".bam"))
  }
  }
  ...
  for(i in 1:length(clusters)){
    ...
    write_log_start(f = "../scAPA.script.log", paste0("samtools merge for ",
                                                      clusters[i]),
                    command = FilterBAMbyTag.command)
...
if(!sc){
  ...
  metadata <- read_down.seq(saf = utr.saf,
   ...
   if(int){
    write_log_start(f = "../scAPA.script.log", "featureCounts for introns", 
                    command = NA)
...
if(sc){
  ...
  for(j in 1:length(samples.vector)) {
    ...
    split_bams <- function(x){
      ...
      FilterBAMbyTag.command <- paste0(drop.seq.tools.path,"FilterBamByTag ",
                                       "TAG=CB I=dedup.", sample, ".bam O=",
                                       "./CellBams/", bam," TAG_VALUE=", cell,
                                       " &>> ../Log.files/",
                                       "FillterBam_forcells.out")
      system(command = FilterBAMbyTag.command, wait = T)
    }
    parallel::mclapply(X = list.cells, FUN = split_bams,
                       mc.cores=c, mc.preschedule=T)
  }
...
if(int){
  write_log_start(f = "../scAPA.script.log", "featureCounts for introns",
                  command = NA)
  counts_int <- featureCounts(files = bam.cluster.files ,isGTFAnnotationFile = F,
                          strandSpecific = 1, annot.ext = int.saf,
                          largestOverlap = T,
                          nthreads = c)
  co_int <- cbind.data.frame(rownames(counts_int$counts), counts_int$counts)
  ...
  a.int <- set_scAPAList(.cells.counts = co_int, .row.Data = meta_int,
                         .down.seq = aseq_int, .cluster.anot = full.list.cells)
...
setwd("../")
 ...
 write.table(x = cells_pui, file = "./outs/Mean.Cell.PPUI.txt", quote = F,
...
write.peaks(.x = reasults, .f = "./outs/UTRs.with.multiple.peaks.txt")
 ...
 write.table(x = cells_pui, file = "./outs/Intronic.Mean.Cell.IPUI.txt", quote = F,
             sep = "\t", col.names = T, row.names = F)
 write.table(x = reasults.output, file = "./outs/Intronic.APA.events.txt", quote = F,
             sep = "\t", col.names = T, row.names = F)
