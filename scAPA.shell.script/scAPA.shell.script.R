...
if (((script.args[1] == "-h") | (script.args[1] == "--help") | (is.na(script.args[1])))) {
    cat("\n Usage: scAPA.script.R -p <path.to.files> -org <organism> -sp ", "<path.to.script.Dir> [options]\n\n", "Basic Options:\n\n\t-p \tThe path to the folder with input bam files,", "and cell cluster annotatios files", 
        " and cell.cluster.list.txt\n\t\t/path/to/files\n\n\t-org \tOrganism,", " either Mm for mouse, or Hs for human\n\n\t-c\tThe number of cores to", "use.\n\t-sp The path to the directory of ", 
        "scAPA.shell.script.\n\n\t-loc\t(Required only if the user does not ", "have permissions to install packages in the default R library).", "Path to users' library trees within which packages are installed and", 
        " looked for.\n\nOutput Options", "\n\n\t-wig\tWeather to generate cluster wig files.", " Defult value: false\n\nAnalysis ", "\n\n\t-ChangePoint\t weather to", " use Change Point. Defult is false.", 
        "Options\n\n\t-sc\tIf true, (default) counts read from individual", " cells (slower). Otherwise, counts reads from ", "clusters.\n\n\t-int\tIf true, ", "(default) perform intronic APA analysis as well as 3'UTR", 
        " analysis.\n\nFiltering", "Options 3'UTRS\n\n\t-cpm\tConsider only peaks with more than", " a total sum of CPMs over all cell clusters larger than ", "-cpm.\n\t\tDefult", " value: 10\n\n\t-a\tNumeric value, filter out peaks with", 
        " -a consecutive As", "in the region -u to -d downstream thire 3' ", "edge.\n\t\tDefult value: 8\n\n\t-u", "\tNumeric value, filter out peaks with -a consecutive As in the region", " -u to -d downstream thire 3' edge.\n\t\tDefult value: ", 
        " 10\n\n\t-d\tNumeric ", "value, filter out peaks with -a consecutive As in the region -u to -d ", "downstream thire 3' edge.", "\n\t\tDefult value: 140", "\n\nFiltering Options introns\n\n\t-Icpm\tConsider only peaks ", 
        "with more than", " a total sum of CPMs over all cell clusters larger than -Icpm.", "\n\n\t-Ico\tConsider only peaks with more than", " a total sum of counts over all cell clusters larger than", 
        " -Ico\n\t\tDefult", " value: 50\n\n\t-Ia\tNumeric value, filter out peaks with -Ia", " consecutive As", "in the region -u to -d downstream thire 3' ", "edge.\n\t\tDefult value: 7\n\n\t-Iu", 
        "\tNumeric value, filter out peaks with -Ia consecutive As in the ", "region -Iu to -Id downstream thire 3' edge.\n\t\tDefult value: ", "1\n\n\t-Id\tNumeric ", "value, filter out peaks with -Ia consecutive As in the region", 
...
if (length(script.args) == 0) {
    stop(paste0("scAPA.script.R -p <path.to.files> -org <organism>.\n", "The following arguments are optional: -cpm <CPM.cuttoff>", "  -a <A.number> -u <filter.border.left> -d ", "<filter.border.right>"), 
        call. = FALSE)
}
if (length(script.args) < 3) {
    stop(paste0("At least three argument must be supplied (path to files ,", "organism \"Hs\"/\"Mm\" and path to scAPA.shell.script folder)"), call. = FALSE)
}
...
if (!(org %in% c("Mm", "Hs"))) {
    stop(paste0("The second argument (organism) must be either Mm ", "(for mouse) or Hs (for human)"), call. = FALSE)
}
...
ICPM.cuttoff <- read_args(arg.string = "-Icpm", defult = 10, arg = script.args)
IA.number <- read_args(arg.string = "-Ia", defult = 7, arg = script.args)
...
# Load R packeges and function scripts------------------------------------------ This function load packeges and install them in case they are not installed
...
log.files.messege <- paste0("\nThe script's log file is written into: ", path.to.files, "/scAPA/scAPA.script.log.\n", "The log files of the tools used are ", "written to the directory: ", path.to.files, 
    "/scAPA/Log.files.\n\n")
...
# Our pipeline consists of the following 5 steps: 1. Defining 3'UTR peaks 2. Quantifying the usage of each peak in each cell cluster 3. Filtering peaks 4. Detecting dynamical APA events 5.
# Inferring global trends of APA modulation

# 1.Defining 3'UTR peaks PCR duplicates removal --------------------------------------------------------------- FilterBAM ---------------------------------------------------------------
write_log_start("Stage 1: Defining 3'UTR peaks\n\n", command = NULL)
write_log_start("Stage 1a: PCR duplicates removal\n\n", command = NULL)
FilterBAM.command <- paste0("sh -c 'for sample in ", samples, " ;", " do ", drop.seq.tools.path, "FilterBam TAG_RETAIN=UB", " I=../${sample}.bam O=./temp/UB.${sample}.bam &> ", "./Log.files/FilterBAM.${sample}.bam.out; done'")
...
# samtools index ----------------------------------------------------------
index.command <- paste0("sh -c 'for sample in ", samples, " ; do ", samtools.path, "samtools index temp/UB.${sample}.bam", " &> ./Log.files/Index.${sample}.out; done'")
...
# umi_tools ---------------------------------------------------------------
umi_tools.command <- paste0("sh -c 'for sample in ", samples, " ;do ", umi_tools.path, "umi_tools dedup -I ", "temp/UB.${sample}.bam -S temp/dedup.${sample}.bam", " --method=unique --extract-umi-method=tag ", 
    "--umi-tag=UB --cell-tag=CB &> ", "./Log.files/umi_tools.${sample}.out;", " done'")
...
# makeTagDirectory --------------------------------------------------------
makeTagDirectory.command <- paste0(homer.path, "makeTagDirectory ./temp/Tagdirectory ", "./temp/dedup.* &> ./Log.files/", "makeTagDirectory.out")
...
# # findPeaks ---------------------------------------------------------------
findPeaks.command <- paste0(homer.path, "findPeaks ./temp/Tagdirectory -size 50 -frag", "Length 100 -minDist 1 -strand separate -o ", "./temp/Peakfile &> ./Log.files/findPeaks.out")
...
peaks.bed <- intersect_peaks(org = org, bed.name = "./merge.peakfile.bed", path = "", bedtools.path = bedtools.path, introns = F)
write.bed(.x = peaks.bed, f = "./peaks.bed")

# Spliting bimodal peaks --------------------------------------------------
write_log_start(paste0("Stage 1c: Separating Peaks with bimodal UMI", " counts distribution\n\n"), command = NA, f = "../scAPA.script.log")
# Samtools merge
dedup.bams <- paste0("dedup.", bams.files, collapse = " ")
samtools.merge.commad <- paste0(samtools.path, "samtools merge merged.bam ", dedup.bams, " &> ../Log.files/samtoolsmerge.out")
write_log_start(f = "../scAPA.script.log", "samtools merge", command = samtools.merge.commad)
...
# Bedgraphs
wig.plus.command <- paste0(bedtools.path, "bedtools genomecov -ibam merged.bam ", "-bg -strand + | awk 'BEGIN {OFS = \"\t\"}{print $1", ", $2, $3, $4, \".\", \"+\"}' > merged.wig")
system(command = wig.plus.command, wait = T)
wig.minus.command <- paste0(bedtools.path, "bedtools genomecov -ibam merged.bam ", "-bg -strand - | awk 'BEGIN {OFS = \"\t\"}{print $1", ", $2, $3, $4, \".\", \"-\"}' >> merged.wig")
system(command = wig.minus.command, wait = T)
system("rm merged.bam", wait = T)

intersect.wig.command <- paste0(bedtools.path, "bedtools intersect -s -wb ", " -b peaks.bed -a merged.wig > intersected.wig")
...
bed <- plyr::rbind.fill(parallel::mclapply(1:length(peaks.wig), FUN = creat_mclus, mc.cores = c, mc.preschedule = T))
if (!exists("bed")) {
...
if (int) {
    merge_peaks(bedtools.path = bedtools.path, peaks.file = "Peakfile", path = "./")
    int.peaks.bed <- intersect_peaks(org = org, bed.name = "./merge.peakfile.bed", path = "", bedtools.path = bedtools.path, introns = T)
...
if (wig | !sc | ChangePoint) {
    ...
    for (j in 1:length(samples.vector)) {
        ...
        for (i in 1:length(list.cells)) {
            ...
            FilterBAMbyTag.command <- paste0(drop.seq.tools.path, "FilterBamByTag ", "TAG=CB I=dedup.", sample, ".bam O=", sample, "_", cluster, ".bam ", "TAG_VALUES_FILE=", tagValuefile, " &> ../Log.files/FilterBamByTag_", 
                tagValuefile)
            print(FilterBAMbyTag.command)
            write_log_start(f = "../scAPA.script.log", paste0("FilterBAMbyTag for ", tagValuefile), command = FilterBAMbyTag.command)
            system(command = FilterBAMbyTag.command, wait = T)
            if (!file.exists(paste0(sample, "_", cluster, ".bam"))) {
                write_log(f = "../scAPA.script.log", stage = paste0("FilterBAMbyTag for ", tagValuefile), success = F)
                stop("FilterBAMbyTag.command did not end, shell script did NOT end!")
            }
            write_log(f = "../scAPA.script.log", stage = paste0("FilterBAMbyTag.command: ", sample, "_", cluster, ".bam"))
        }
    }
    clusters <- unique(allclusters)
    for (i in 1:length(clusters)) {
        bams.to.merge <- paste0(samples.vector, "_", clusters[i], ".bam", collapse = " ")
        samtools.merge.command <- paste0("samtools merge ", clusters[i], ".bam ", bams.to.merge)
        print(samtools.merge.command)
        write_log_start(f = "../scAPA.script.log", paste0("samtools merge for ", clusters[i]), command = FilterBAMbyTag.command)
        system(command = samtools.merge.command, wait = T)
        write_log(f = "../scAPA.script.log", stage = paste0("samtools merge for ", clusters[i]))
        if (wig) {
            wig.command <- paste0(bedtools.path, "bedtools genomecov -trackline -bg ", "-trackopts \"name=", clusters[i], "-ibam ", clusters[i], ".bam > ../outs", clusters[i], ".wig")
            print(wig.command)
            write_log_start(f = "../scAPA.script.log", paste0("bedtools genomecov for ", clusters[i]), command = wig.command)
            system(command = wig.command, wait = T)
            write_log(f = "../scAPA.script.log", stage = paste0("bedtools genomecov for ", clusters[i]))
        }
    }
...
if (!sc | ChangePoint) {
    write_log_start(f = "../scAPA.script.log", "featureCounts for 3UTRs", command = NA)
    bam.cluster.files <- paste0(clusters, ".bam", collapse = " ")
    counts <- featureCounts(files = bam.cluster.files, isGTFAnnotationFile = F, strandSpecific = 1, annot.ext = utr.saf, largestOverlap = T, nthreads = c)
    ...
    metadata <- read_down.seq(saf = utr.saf, char.length.path = char.length.path, fasta.path = fasta.path, chr.modify = T)
    aseq <- metadata[, c(4, 6)]
    ...
    if (int) {
        write_log_start(f = "../scAPA.script.log", "featureCounts for introns", command = NA)
        counts_int <- featureCounts(files = bam.cluster.files, isGTFAnnotationFile = F, annot.ext = int.saf, strandSpecific = 1, largestOverlap = T, nthreads = c)
        ...
        metadata_int <- read_down.seq(saf = int.saf, char.length.path = char.length.path, fasta.path = fasta.path, chr.modify = T)
        ...
        write_log(f = "../scAPA.script.log", stage = "featureCounts for introns")
    }
}
...
# ChangePoint -------------------------------------------------------------
if (ChangePoint) {
    allcombintations <- paste0(" -t ", bam.cluster.files, " -c ", rep(bam.cluster.files, each = length(bam.cluster.files)))
    toexclude <- seq(from = 1, to = length(allcombintations), by = (length(bam.cluster.files) + 1))
    allcombintations <- allcombintations[-toexclude]
    if (org == "Mm") 
        write.bed(.x = mm10.utr, f = "3UTR.BED")
    if (org == "Hs") 
        write.bed(.x = hg19.utr, f = "3UTR.BED")
    allcombintations_names <- gsub(pattern = " -t ", replacement = "", x = allcombintations)
    allcombintations_names <- gsub(pattern = " -c ", replacement = "_vs_", x = allcombintations_names)
    allcombintations_names <- paste0("ChangePoint.", allcombintations_names)
    ChangePoint.command <- paste0("perl ", path.to.chang.point, "change_point.pl -g 3UTR.BED", allcombintations, " -d s -o ", allcombintations_names, " &> ../Log.files/", allcombintations_names, 
        ".out")
...
if (sc) {
    ...
    for (j in 1:length(samples.vector)) {
        ...
        full.list.cells <- rbind.data.frame(full.list.cells, list.cells[, c(3, 2)])
        list.cells <- split(x = list.cells, f = list.cells$V1, drop = T)
        split_bams <- function(x) {
            cell <- x[, 1]
            bam <- paste0(x[, 3], ".bam")
            FilterBAMbyTag.command <- paste0(drop.seq.tools.path, "FilterBamByTag ", "TAG=CB I=dedup.", sample, ".bam O=", "./CellBams/", bam, " TAG_VALUE=", cell, " &>> ../Log.files/", "FillterBam_forcells.out")
            system(command = FilterBAMbyTag.command, wait = T)
        }
        parallel::mclapply(X = list.cells, FUN = split_bams, mc.cores = c, mc.preschedule = T)
    }
    ...
    write_log_start(f = "../scAPA.script.log", "featureCounts for 3UTRs", command = NA)
    bam.cluster.files <- list.files(path = "./CellBams", pattern = ".bam$", full.names = F)
    cellnames <- gsub(x = bam.cluster.files, pattern = ".bam", replacement = "")
    bam.cluster.files <- list.files(path = "./CellBams", pattern = ".bam$", full.names = T)
    counts <- featureCounts(files = bam.cluster.files, isGTFAnnotationFile = F, strandSpecific = 1, annot.ext = utr.saf, largestOverlap = T, nthreads = c)
    co <- cbind.data.frame(rownames(counts$counts), counts$counts)
    ...
    metadata <- read_down.seq(saf = utr.saf, char.length.path = char.length.path, fasta.path = fasta.path, chr.modify = T)
    aseq <- metadata[, c(4, 6)]
    a <- set_scAPAList(.cells.counts = co, .row.Data = meta, .down.seq = aseq, .cluster.anot = full.list.cells)
    rm(list = c("co", "meta", "aseq"))
    saveRDS(object = a, file = "../outs/Peaks.RDS")
    write_log(f = "../scAPA.script.log", stage = "featureCounts for 3UTRs")
    if (int) {
        write_log_start(f = "../scAPA.script.log", "featureCounts for introns", command = NA)
        counts_int <- featureCounts(files = bam.cluster.files, isGTFAnnotationFile = F, strandSpecific = 1, annot.ext = int.saf, largestOverlap = T, nthreads = c)
        co_int <- cbind.data.frame(rownames(counts_int$counts), counts_int$counts)
        ...
        metadata_int <- read_down.seq(saf = int.saf, char.length.path = char.length.path, fasta.path = fasta.path, chr.modify = T)
        aseq_int <- metadata_int[, c(4, 6)]
        a.int <- set_scAPAList(.cells.counts = co_int, .row.Data = meta_int, .down.seq = aseq_int, .cluster.anot = full.list.cells)
...
a.fil <- filter_IP(x = a.fil, int.priming.seq = IP.seq, left = filter.border.left, right = filter.border.right)
...
cells_pui$cell <- gsub(pattern = "_proximal_PUI", replacement = "", x = rownames(cells_pui))
cells_pui$sample <- gsub(pattern = "_.*", replacement = "", x = cells_pui$cell)
cells_pui$cell <- gsub(pattern = ".*_", replacement = "", x = cells_pui$cell)
cells_pui <- cells_pui[, c(3, 2, 1)]
colnames(cells_pui) <- c("Sample", "Cell_BC", "Mean_Proximal_PUI")
write.table(x = cells_pui, file = "./outs/Mean.Cell.PPUI.txt", quote = F, sep = "\t", col.names = T, row.names = F)

write.table(x = reasults.output, file = "./outs/APA.events.txt", quote = F, sep = "\t", col.names = T, row.names = F)

write.table(x = a.fil@row.Data, file = "./outs/ThreeUTR.peaks.txt", quote = F, sep = "\t", col.names = T, row.names = F)

write.peaks(.x = reasults, .f = "./outs/UTRs.with.multiple.peaks.txt")

file.create("./outs/summary.txt")
cat("Number of peaks passed fillter:\t", nrow(a.fil@clus.counts), "\nNumber of significant (FDR < 5%) APA events:", sum(reasults@pvalues[[1]][, 2] < 0.05, na.rm = T), "\n", file = "./outs/summary.UTR.txt")
...
colnames(tidy.ppui) <- gsub(x = colnames(tidy.ppui), pattern = "_proximal_PUI", replacement = "")
tidy.ppui <- tidyr::gather(data = tidy.ppui)
pdf("./outs/Proximal.PUI.ECDF.pdf")
p <- ggplot2::ggplot(data = tidy.ppui, ggplot2::aes(x = value, color = key))
...
if (int) {
    ...
    reasults <- set_scAPAreasults(x = a, int = T, cpm = ICPM.cuttoff, counts = IC.cuttoff)
    
    # Internal priming filtering
    IP.seq.I <- paste0(rep("A", times = IA.number), collapse = "")
    reasults <- filter_IP(x = reasults, int.priming.seq = IP.seq.I, left = Ifilter.border.left, right = Ifilter.border.right)
    ...
    write.table(x = cells_pui, file = "./outs/Intronic.Mean.Cell.IPUI.txt", quote = F, sep = "\t", col.names = T, row.names = F)
    write.table(x = reasults.output, file = "./outs/Intronic.APA.events.txt", quote = F, sep = "\t", col.names = T, row.names = F)
    write.table(x = reasults@metadata, file = "./outs/Intronic.peaks.txt", quote = F, sep = "\t", col.names = T, row.names = F)
    file.create("./outs/summary.txt")
    cat("Number of peaks passed fillter:\t", nrow(a.fil@clus.counts), "\nNumber of significant (FDR < 5%) APA events:", sum(reasults@pvalues[[1]][, 2] < 0.05, na.rm = T), "\n", file = "./outs/summary.UTR.txt")
    ...
    p <- ggplot2::ggplot(data = tidy.ppui, ggplot2::aes(x = value, color = Cluster))
    p <- p + ggplot2::stat_ecdf(size = 1)
    p <- p + ggplot2::theme_bw()
