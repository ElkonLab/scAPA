...
if (((script.args[1] == "-h") | (script.args[1] == "--help") | (is.na(script.args[1])))) {
    cat("\n Usage: scAPA.script.R -p <path.to.files> -org <organism> -sp ", 
        ...
        "scAPA.shell.script.\n\n\t-loc\t(Required only if the user does not ", 
        "have permissions to install packages in the default R library).", 
        "Path to users' library trees within which packages are installed and", 
        " looked for.\n\nOutput Options",
...
if (!sc | ChangePoint) {
    ...
    if (int) {
        ...
        counts_int <- featureCounts(files = bam.cluster.files, 
