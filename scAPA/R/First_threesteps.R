...
#' Intersect peaks bed file with a 3'UTR or introns
#'
...
#' @param bedtools.path The path to bedtools. (e.g "/path/to/bedtools/bin/".
#' End with "/", leave "" if bedtools is in your PATH environment variable)
#' @param introns Whether to intersect with introns or not (3'UTRs).
...
#' Detects and separates bimodal peaks
#'
...
#'
#' @return A 'BED format' data.frame. If the peak is split, than the
...
#' Returns the genomic sequence downstream the 3' edge of a peak
#'
#' This function uses bedtools getfasta to returns the 3'
#' edge of a peak
#'
#' @param saf A dataframe, SAF (Simplified Annotation Format) of the peaks.
#' @param char.length.path The path to the chromosome length file.
#' @param fasta.path The path to the fasta file
#' @param chr.modify Whether the chromosome length file has "chr"
#' in the chromosome names (chr1 and not 1)
#' @param l extract the sequence between l upstream and r
#' downstream the peak 3' edge. Default -1
#' @param r extract the sequence between l upstream and r
#' downstream the peak 3' edge. Default 200
#' @return A data.frame with two columns: one is peak ID and the other
#'  is the downstream sequence.
